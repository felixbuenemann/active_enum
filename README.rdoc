...
The +to_select+ method looks up translations for the enum values in +active_enum+. The translation key is
built from the enum class name and the enum value's name attribute downcased and underscored.
...
The enum class also includes +translate+ method (also aliased as +t+) for translating the enum values. The translation
can be looked up by either id or name.
